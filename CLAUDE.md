...
| `APP_DATEV_LOHN_ENABLED` | `false` | Schaltet den **DATEV-Lohn-Export** (LODAS/Lohn&Gehalt Bewegungsdaten, Plan `plan/ausbau-wawi-datev-personal-reporting.md` PERSONAL-2/3) frei. **Kein Secret** — nur Sichtbarkeits-Schalter (`AppConfig.datevLohnEnabled`); Button nur bei `datevLohnEnabled && isAdmin`. Default aus bis Format + Lohnartnummern vom Steuerberater bestätigt. |
...
8b. **Neuer Hybrid-Schreibpfad (Q1, Offline-Positivliste)** → NIE „bei jedem Fehler lokal zurückfallen". Den lokalen Fallback NUR bei echten Offline-Fehlern greifen lassen (`lib/core/hybrid_write_fallback.dart` `HybridWriteFallback.tryFirestoreWrite` bzw. `isTransientError` aus `retry.dart`: nur `unavailable`/`deadline-exceeded`/`TimeoutException`). ALLES andere — `permission-denied`/`unauthenticated` (Rules-Deny), `failed-precondition` (fehlender Composite-Index sieht sonst wie „offline" aus!), `invalid-argument`, … — MUSS sichtbar scheitern (rethrow, KEIN lokaler Write, KEIN Audit), sonst unterläuft der Fallback jede Rules-Härtung. Finance/Personal/Inventory mixen `HybridWriteFallback` ein (`_tryFirestore` delegiert). **Test-Konvention:** Offline-Fallback in Tests IMMER mit `FirebaseException(code:'unavailable')` simulieren (NICHT `Exception('offline')` — das rethrowt jetzt korrekt); je neuem Hybrid-Mutator ein Deny-Test „permission-denied → Fehler sichtbar, kein lokaler Write".
