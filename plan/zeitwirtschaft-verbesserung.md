...
**Stand:** 2026-07-03 · **Status:** Umsetzung erledigt (ZV-1…ZV-7 code-fertig+getestet; nur Deploy + bewusst vertagte Reste offen) · **Priorität:** sehr hoch
...
| ZV-1 | Konnektivität ehrlich + Pending-Transparenz + Resume | **komplett** — ZV-1.1/1.2/1.4 gebaut+getestet; ZV-1.1-Probe-Mechanik dokumentiert (opt-in, keine neue Dependency); **ZV-1.3 entschieden+dokumentiert**: Web bleibt Single-Tab (cloud_firestore-Flutter-Plugin hat keinen Multi-Tab-Manager), zweiter Tab degradiert graceful auf online |
| ZV-2 | Schichtplan ↔ Stempeln (Kontext, Soll-Ist, Auto-Klärung) | **komplett gebaut+getestet** (shiftId App+Kiosk, dienst_abgleich-Kern, Dienst-heute-Karte, autoKlaerungNightly, **Kiosk-Schichtauflösung** server-seitig in `ServerKioskClockService` + `source='kiosk'`) |
...
| ZV-5 | Payroll-Kette schließen + Abschluss-Arbeitsliste | **ZV-5.2 + ZV-5.3 gebaut+getestet** (Klärungs-Blocker; §3b-Transparenz via purem `sfn_lage.dart`-Kern + Stundenkonto-Karte, 8 Tests); **ZV-5.1 teils** (Schicht-Ketten-Test shiftId→sourceShiftId); der „abgelehnt→ClockEntry"-Hinweis bleibt vertagt (bräuchte ClockEntry↔WorkEntry-Join in der Monatsliste) |
...
| ZV-7 | Push-Delta „Zeit-Ereignisse" | **komplett gebaut+getestet** — Auto-Klärung-Push (Nightly) + `onClockEntryWritten`-Trigger „Klärung gelöst → Mitarbeiter" (nur Übergang klaerung→completed, kein Spam), `buildKlaerungResolvedNotification` (functions 32 grün) |
| ZV-8 | Deploy & Verifikation (gebündelt) | **rules+indexes+functions LIVE auf `taskmaster-ebcez` (03.07., `onClockEntryWritten` neu angelegt, alle Callables/Trigger aktualisiert)**; OFFEN: Storage (einmalige Console-Aktivierung nötig) + Hosting (`flutter build web` mit Firebase-Config) + `rebuildPosDailyStats`-Backfill als Admin + Commit + On-Device-Abnahme |
