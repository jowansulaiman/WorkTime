...
### WW-8: Persistente Inventur-Sessions (anlegen, unterbrechen, fortsetzen, mehrgerätefähig) — Status: ERLEDIGT (14.07., 0897604 Datenschicht+Rules, cd4e34b UI)
> Model (append-only Events + InventoryCountDiff), Repo (lines-Subcollection, kein Index), DatabaseService (org-skopierte Metadaten + chunkbare per-Session-Lines-Keys), Provider (start/record/cancel/resume/loadCountLines/conflictsFor), Rules-Transition-Matrix (open→completed/cancelled terminal, Akteur-Pin, bookedAt-only-Update via diff().affectedKeys()), UI additiv über /inventur (Session starten/fortsetzen, debounced recordCount, Abschließen), 20+5 Tests. **Offen:** Chunking-Perf-Test 3k/10k (Local-Monolith→per-Session-Key ist da, Messung dokumentieren); Deploy firestore:rules.
...
### WW-9: Inventur-Abschluss — Konflikt-Auflösung, Differenzbuchung mit Session-Bezug + Historie — Status: ERLEDIGT (14.07., bf5dc21 Provider-Kern, cd4e34b UI)
> completeCountSession (Konflikt-/Stale-Gates, absolute Buchung via recordStocktake mit deterministischer mutationId + externalRef, bookedAt-Markierung, diffSummary-Freeze), loadInventurMovements (index-frei), Abschluss-UI mit Konflikt-/Stale-Dialogen. 6 Tests.
...
### WW-10: Inventur-Protokoll als PDF/CSV + GoBD-Härtung — Status: ERLEDIGT (14.07., 8ab6a7d CSV, PDF-Commit)
> buildInventoryCountCsv (UTF-8-BOM + ;) + generateInventoryCountReport (PDF) + Export-Wrapper + „Abgeschlossene Inventuren"-Sheet mit CSV/PDF-Export; Inhalt IMMER aus persistierten Daten (diffSummary/lines), nie live neu bewertet; includeValuation gated. GoBD-Unveränderlichkeit über die WW-8-Rules (completed terminal). 3 CSV- + 1 PDF-Test. **Offen:** Deploy firestore:rules.
