...
| **P-6** | **Ausgeben-Flow** (Scan/Kundenhandy/Name → gebündelte Karte → „Ausgegeben"/„Alle" + Bestätigung>1) + Status/`handedOutAt` + abgeleitete Fach-Freigabe + **Undo/Recovery** (`clearHandedOutAt`, Re-Bindung, Konflikt) + Hinweisbanner + Leer-/Nacherfassen-Zustand | Test: Status→`abgeholt`, `handedOutAt` gesetzt, Fach abgeleitet frei; Undo stellt `eingelagert`+Fach wieder her; Teilausgabe hält Fach belegt; Bündelung | offen |
