# Paketshop — Sortieren & Ausgeben (carrier-agnostisch)
...
- **Überblick:** Fach-Auslastung (frei/belegt), Überfällig-/Ladenhüter-Liste (konfigurierbare Frist), Reverse-Lookup „was liegt in Fach X?", **Tages-Reconciliation** (heute angenommen / ausgegeben) zum Abgleich mit dem offiziellen Anbieter-Gerät, **Rücklauf-Sammelaktion**.

**Abgrenzung (ehrlich):** Unser Modul ersetzt **NICHT** den offiziellen Prozess des Paketdiensts. Das offizielle Annehmen/Ausgeben (Hermes-Scanner/PADEA, Ausweisprüfung, Unterschrift) macht das Personal weiterhin am Anbieter-Gerät. Wir sind ein **paralleles internes Bestandsregister**. Die UI kommuniziert das per persistentem Hinweisbanner unmissverständlich.
...
| **Kundenbenachrichtigung** | Der jeweilige Paketdienst benachrichtigt selbst (E-Mail/App). | — | v1 benachrichtigt **nicht** (nur intern). Spätere Ausbaustufe. |
...
- **Carrier-agnostisch** (Betreiber-Wunsch 13.07.): beliebige Paketdienste (DHL/Hermes/DPD/GLS/UPS …). `carrier` ist ein **optionales** Feld je Paket (freier String, `null` = nicht angegeben), kein fixer Anbieter.
- Nur **Standort Tabak Börse** (siteId-Pflichtfeld; Strichmännchen ist kein Paketshop). Herkunft der siteId: aus `TeamProvider.sites`, aufgelöst über `paketshopSiteId` im Config-Doc (s. §6.6/§7).
...
- **Config-Doc** `config/paketshopSettings` (Überfällig-Frist, `paketshopSiteId`, optionaler Anonymisierungs-Schalter **Default aus**) **(neu)**.
...
- ❌ **Carrier-spezifische** Features (dienst-eigene Barcode-Validierung/Tracking-API-Anbindung/Retouren-Import) — v1 behandelt alle Codes als opake Strings; `carrier` ist nur ein optionales Label/Filter.
...
5. **Bestätigung:** grünes Feedback (`ScanFeedback.success` + visueller Blitz), **Toast** „Paket für *Schmidt* in Fach **A2** eingelagert". Status = `eingelagert`, `arrivedAt = now`, `siteId`/`siteName` = aufgelöster Paketshop-Standort. **Audit-Summary personenfrei:** „Paket eingelagert (Fach A2)" — **nie** mit Empfängernamen (s. §14).
...
5. **„Ausgegeben" bestätigen:** ein Tap (Einzel oder „Alle ausgeben").
   ...
   - **Prominenter Hinweisbanner:** „Offizielle Hermes-Ausgabe (Ausweis + Unterschrift am Anbieter-Gerät) zusätzlich zwingend."
...
String   siteId                // Pflicht (Tabak Börse); Herkunft s. §7 (aus TeamProvider.sites via paketshopSiteId)
String?  siteName              // Cache (clearSiteName) — im Mutator frisch gesetzt, nicht via copyWith durchreichen
String?  carrier               // OPTIONAL, freier String (DHL/Hermes/DPD/…); null = nicht angegeben
...
String   id
String   orgId
String   siteId                // Paketshop-Standort (Tabak Börse)
String   firstName
String   lastName
...
String? paketshopSiteId               // aktiver Paketshop-Standort (Tabak Börse) → siteId-Auflösung §7
```
...
- **Kein TeamProvider-Dependency** (ParcelProvider bleibt Proxy3). Die **Pflicht-siteId** kommt **nicht** aus dem Provider: Screens lesen `context.read<TeamProvider>().sites`, wählen den Standort mit `id == paketshopSettings.paketshopSiteId` (bzw. den einzigen Standort, falls die Org nur einen hat) und reichen **`siteId` + `siteName` in `saveParcel`/`saveCompartment` durch** (Major-Fix). `siteName` ist reiner Anzeige-Cache.
...
- **Hub-Einbindung:** Kachel „Paketshop" im `_ShopHubTab` (`home_screen`, ~Zeile 2875+, **korrigierter Name**) mit Badge „X offen / Y überfällig". Optional zusätzlich Home-Schnellaktion „Paket annehmen".
...
| **PaketAusgebenScreen** (oder Sheet) | dito | Flow §5b. SearchBar (Name/Teil-Nr.) + „Scannen" + **„Kundenhandy-Code scannen"**. Gebündelte Kunden-Karte (Dubletten-Zusatzinfo inkl. Absender). „Ausgegeben"/„Alle" (+ Bestätigung >1) + Undo-Snackbar + Hinweisbanner + Leer-Zustand/Nacherfassen. |
...
6. **Fach-Auslastung** + **Reverse-Lookup** + **Tages-Reconciliation** (heute angenommen/ausgegeben — Abgleich mit dem Anbieter-Gerät).
...
| **P-3** | Provider in `main.dart`-Kette (nach Audit) + Permission-Getter (`canViewParcels`/`canManageParcels` = **`isActive`, alle aktiven Mitarbeiter §0**) + Route `/paketshop` + `route_permissions`-Case + Hub-Kachel; **siteId/siteName-Auflösung via `TeamProvider.sites` + `paketshopSiteId`** | App startet offline; Route erreichbar/gegatet; Router-Test via `pumpApp`; siteId wird durchgereicht | ✅ **erledigt (13.07.)** — Provider (Proxy3 nach Audit, `surfaceSessionError`), `canViewParcels`/`canManageParcels`=`isActive` (app_user), Gate-Case (`route_permissions`), `AppRoutes.paketshop`+`_sectionRoute` (app_router), Hub-Kachel im `_ShopHubTab` (Badge offen/überfällig), neuer `PaketshopHubScreen` (Kennzahlen + Hinweisbanner + siteId-Auflösung `TeamProvider.sites`+`paketshopSiteId`); volle Suite 2039 grün, analyze 0. **Abw.:** Full-Shell-`pumpApp`-Routentest verworfen (Section-Route als Initial-Location hängt im Harness — Repo testet Section-Routes generell nicht so); stattdessen isolierter Hub-Render-Widgettest + reiner `RoutePermissions`-Gate-Unittest (`test/parcel_route_test.dart`) |
| **P-4** | Scan-Primitive extrahieren → `barcode_scan_field.dart`/`showBarcodeScanSheet` (`ScannerTarget.extended`, **kein logistics**, fortlaufende Session); `ScannerScreen` umstellen | Widget-Test mit `_FakeBarcodeScanner.emit()` + `NoopScanFeedback`; code128/qr dekodiert; `scanWindow` nie gesetzt | ✅ **erledigt (13.07.)** — `lib/widgets/barcode_scan_field.dart`: `showBarcodeScanSheet(ctx,{target,title,hint,scanner,feedback}) → Future<String?>` über die Engine-Seams (`BarcodeScanner`/`MobileScannerAdapter`/`ScanFeedback`), `ScannerTarget.extended`, **kein `scanWindow`**, roher Code (keine Retail-Prüfung), Torch/Zoom/Foto/Manuell, 1000 ms-Dedup; `test/barcode_scan_field_test.dart` (emit + Manuell) grün. **Abw.:** `ScannerScreen` NICHT migriert (bewusst später/kollisionsarm — die Engine ist bereits der geteilte Seam, es wird KEINE Kamera-Engine dupliziert, nur eine dünne UI ergänzt) |
| **P-5** | **Einlagern-Flow** (Scan Paket → Scan Fach → Name-Typeahead **aus Register + offenen Paketen**) inkl. Duplikat-Schutz, Fremd-Doppelbelegungs-Warnung, „Neu anlegen" (legt `ParcelCustomer` an), optional Absender | Widget-Test: Paket gebunden, Fremdbelegung warnt, gleicher Kunde nicht; „Neu anlegen" erzeugt Register-Eintrag; Fuzzy-Match-Core-Test | ✅ **erledigt (13.07.)** — `lib/screens/paket_einlagern_screen.dart` (Scan Paket [Duplikat-Warnung] → Scan/Anlegen Fach [Belegungs-Warnung + Freifach-Vorschlag] → Empfänger-Typeahead über Register + „Neu anlegen"→`upsertCustomer` → optional Absender + **Paketdienst/`carrier`**), Hub-Button „Paket annehmen" (gegatet auf aufgelösten Standort), Einlagern-Button in persistenter `bottomNavigationBar`; `test/paket_einlagern_screen_test.dart` (voller Flow) grün, volle Suite 2047 grün, analyze 0. **Abw.:** Fremd-Belegungs-Warnung beim Fach-Scan (Empfänger folgt danach); Typeahead normalisierter Teilstring (echte Fuzzy später); Scanner injizierbar für Test; großer Test-Viewport wg. langem Formular |
| **P-6** | **Ausgeben-Flow** (Scan/Kundenhandy/Name → gebündelte Karte → „Ausgegeben"/„Alle" + Bestätigung>1) + Status/`handedOutAt` + abgeleitete Fach-Freigabe + **Undo/Recovery** (`clearHandedOutAt`, Re-Bindung, Konflikt) + Hinweisbanner + Leer-/Nacherfassen-Zustand | Test: Status→`abgeholt`, `handedOutAt` gesetzt, Fach abgeleitet frei; Undo stellt `eingelagert`+Fach wieder her; Teilausgabe hält Fach belegt; Bündelung | ✅ **erledigt (14.07.)** — `lib/screens/paket_ausgeben_screen.dart` (Namenssuche + Scan/Kundenhandy-Code → gebündelte Kunden-Karte → „Ausgegeben"/„Alle N" + Bestätigung>1 + **Undo**-Snackbar; Provider `handOutParcel`/`undoHandOut`, `compartmentId` bleibt erhalten); Hub-Button „Paket ausgeben"; `test/paket_ausgeben_screen_test.dart` 2 grün |
| **P-7** | **Fach-Verwaltung** (anlegen mit Barcode-Scan, je-Standort-Eindeutigkeit, Reverse-Lookup, Löschguard belegter Fächer, Umbenennen zieht `compartmentLabel` nach) | Test: doppelter Barcode abgelehnt; belegtes Fach nicht löschbar; Reverse-Lookup korrekt | ✅ **erledigt (14.07.)** — `lib/screens/fach_verwaltung_screen.dart` (anlegen via `_CreateFachDialog` + Barcode-Scan/manuell, je-Standort-Eindeutigkeit im Provider, Reverse-Lookup, Löschguard belegter Fächer); Hub-Button „Fächer verwalten"; `test/fach_verwaltung_screen_test.dart` 2 grün. **Abw.:** Umbenennen (Label-Cache-Nachzug) v1 weggelassen |
| **P-8** | **Überfällig-Board + Auslastung + Rücklauf-Sammelaktion + Tages-Reconciliation** (Frist aus Config, Kalendertage) + Hub-Kennzahlen | Test: `overdueParcels(fristTage)` an Grenzdaten (feste `de_DE`-Testdaten); Rücklauf-Bulk setzt Status; Reconciliation-Filter | ✅ **erledigt (14.07.)** — `lib/screens/paket_uebersicht_screen.dart` (Tabs Überfällig/Heute: Überfällig-Liste + `returnParcel` einzeln + **Rücklauf-Sammelaktion**; Tages-Reconciliation angenommen/ausgegeben); Hub-Button + KPI-Chips; `test/paket_uebersicht_screen_test.dart` grün |
| **P-9** | **UI-Politur** (mobil/wide, daumenreichbare Aktionen, `appColors`-Chips, leere Zustände, Feedback-Blitz) + **Kunden-Register-Verwaltung** („Kunde löschen"/„Vorgang löschen" = Widerspruch/Art. 17) — **keine Auto-Anonymisierung in v1** (§0) | Test: „Kunde löschen" entkoppelt `parcelCustomerId`; „Vorgang löschen" entfernt Vorgang; Register bleibt bei Vorgangs-Löschung erhalten | ✅ **erledigt (14.07.)** — `lib/screens/kunden_register_screen.dart` (suchbare Liste + **„Kunde löschen"** Art. 17/21 → `deleteCustomer` entkoppelt offene Pakete + Transparenzhinweis); Hub-Button „Kundenregister"; `test/kunden_register_screen_test.dart` grün. **Abw.:** „Vorgang löschen" via `deleteParcel` im Provider vorhanden, dedizierter UI-Einstieg + weitere UI-Politur folgen |
| **P-10** | `firestore.rules`-Blöcke (`parcelShipments`/`shelfCompartments`/**`parcelCustomers`**) + Deploy-Doku; End-to-End-Offline-Durchlauf | Rules greifen (sameOrg + canManageParcels=isActive + orgId-Pin); Deploy-Schritte gelistet; Gesamt-DoD (§16) | ✅ **Code erledigt (14.07.)** — `firestore.rules`: `canManageParcels()`=`isActiveUser()` + Match-Blöcke `parcelShipments`/`shelfCompartments`/`parcelCustomers` (sameOrg + aktiver Nutzer + orgId-Pin); `config/paketshopSettings` via generischen config-Block. **Deploy `firebase deploy --only firestore:rules` bleibt extern/offen** |
...
- [ ] Provider in `main.dart`-Kette nach Audit; `setAuditSink` verdrahtet; **siteId/siteName aus `TeamProvider.sites` + `paketshopSiteId` durchgereicht**.
...
- [ ] Persistenter UI-Hinweis „offizieller Ablauf des Paketdiensts zusätzlich zwingend" vorhanden.
