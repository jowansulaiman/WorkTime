...
import 'package:worktime_app/screens/personal/tabs/employee_ausbildungen_tab.dart';
  ...
  await tester.pumpWidget(
    ChangeNotifierProvider<PersonalProvider>.value(
      ...
      child: MaterialApp(
        ...
        home: const Scaffold(body: EmployeeAusbildungenTab(userId: 'admin-1')),
      ),
    ),
  );
  await tester.pump();
...
void main() {
  ...
  testWidgets('leer: Zähler-Kopf + Hinzufügen + Leerzustand', (tester) async {
    await _pump(tester);
    expect(find.text('0 Ausbildungen'), findsOneWidget);
    expect(find.text('Ausbildung hinzufügen'), findsOneWidget);
    expect(find.text('Keine Ausbildungen vorhanden'), findsOneWidget);
  });

  testWidgets('zeigt Ausbildung mit Status-Badge', (tester) async {
    final personal = await _pump(tester);
    await personal.saveEmployeeAusbildung(const EmployeeAusbildung(
      orgId: '',
      userId: 'admin-1',
    ...
    expect(find.text('1 Ausbildungen'), findsOneWidget);
    expect(find.text('Kaufmann im Einzelhandel'), findsOneWidget);
